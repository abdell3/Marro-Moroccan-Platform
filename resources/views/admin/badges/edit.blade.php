<x-layouts.app :title="'Modifier un badge | Marro'">
    <div class="bg-gray-100 dark:bg-gray-900 min-h-screen">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Modifier un badge</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Édition du badge "{{ $badge->name }}"
                    </p>
                </div>
                <div>
                    ...
                    <form action="{{ route('admin.badges.update', $badge->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" name="nom" id="nom" value="{{ old('nom', $badge->name) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                                @error('nom')
                            ...
                            <div>
                                ...
                                <textarea name="critere" id="critere" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">{{ old('critere', $badge->criteria) }}</textarea>
                                @error('critere')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="logo" id="logo" value="{{ old('logo', $badge->icon) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
